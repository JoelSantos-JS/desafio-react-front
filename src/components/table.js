...
function Table() {
    ...
    const [inicio, setInicio] = useState(); // data inicial
    const [fim, setFim] = useState(); // data final
    ...
    }, [inicio, fim, nomeOperador, transferencia]);  
    ...
    const handleInitDate = (e) => {
        setInicio(e.target.value.toLocaleString('pt-BR'));
    };

    const handleFinDate = (e) => {
        setFim(e.target.value.toLocaleString('pt-BR'));
    };
